...
import { inviteProfile, updateProfile, saveStage, moveStage, saveSource, saveBranding, deleteTag } from "@/lib/actions/settings";
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [team, stages, sources, tags, org, audit] = await Promise.all([
    db.select().from(profilesTable).where(eq(profilesTable.orgId, session.orgId)).orderBy(profilesTable.fullName), listStages(session.orgId), listSources(session.orgId), listTags(session.orgId),
      ...
      <div className="mt-4">
                ...
                <Card key={p.id}>
                  <CardBody>
                    <ActionForm action={updateProfile.bind(null, p.id)} submitLabel="Save" variant="outline" size="sm" className="grid gap-2 sm:grid-cols-6 items-end">
                      <Field label="Name" className="sm:col-span-2"><Input name="fullName" defaultValue={p.fullName} disabled={!isAdmin} /></Field>
                      <Field label="Email" className="sm:col-span-2"><Input name="email" defaultValue={p.email} disabled={!isAdmin} /></Field>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="space-y-4">
              ...
              <Card><CardHeader title="Lead sources" /><CardBody>
                <ul className="text-[13px] space-y-1 mb-3">{sources.map((s) => <li key={s.id} className="flex justify-between"><span>{s.name}</span><span className="text-fg-3 num">{s.costPerLead ? `$${s.costPerLead} per lead` : ""}</span></li>)}</ul>
                {isAdmin ? <ActionForm action={saveSource} submitLabel="Add source" variant="outline" size="sm" resetOnSuccess className="space-y-2"><Input name="name" placeholder="Source name" required /><Input name="costPerLead" type="number" placeholder="Cost per lead" /></ActionForm> : null}
              </CardBody></Card>
            </div>
          </div>
        ) : null}
        ...
        {tab === "branding" ? (
          <Card className="max-w-2xl"><CardHeader title="Branding" description="Used on deal packages and the share page." /><CardBody>
            <ActionForm action={saveBranding} submitLabel="Save branding" className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Email"><Input name="email" defaultValue={branding.email ?? ""} disabled={!isAdmin} /></Field>
              ...
              <Field label="Logo URL"><Input name="logoUrl" defaultValue={branding.logoUrl ?? ""} disabled={!isAdmin} /></Field>
        ...
        {tab === "audit" ? (
          <Card><CardHeader title="Audit log" description="Last 100 changes" /><CardBody className="p-0">
            <ul className="divide-y divide-border text-[13px]">{audit.map(({ a, actor }) => <li key={a.id} className="px-4 py-2 flex gap-3"><span className="text-fg-3 w-32 shrink-0">{dateTime(a.at)}</span><span className="w-32 shrink-0">{actor ?? "System"}</span><span className="text-fg-2">{a.action} {a.entityType} <span className="text-fg-3 font-mono text-[11px]">{a.entityId.slice(0, 8)}</span></span></li>)}</ul>
          </CardBody></Card>
        ) : null}
      </div>
    </>
