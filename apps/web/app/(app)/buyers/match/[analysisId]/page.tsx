...
import { money, fullName, percent, cn } from "@/lib/utils";
...
export default async function MatchPage({ params, searchParams }: { params: Promise<{ analysisId: string }>; searchParams: Promise<{ package?: string }> }) {
  ...
  const deal = { state: detail.property.state, county: detail.property.county, postalCode: detail.property.postalCode, propertyType: detail.property.propertyType, condition: detail.property.condition, occupancy: detail.property.occupancy, investorBuyPrice: out.wholesale?.investorBuyPrice ?? 0, arv: Number(detail.analysis.arv ?? 0), repairCosts: out.acquisitions?.repairCosts ?? 0, spread: Number(detail.analysis.spread ?? 0) };
  const matches = await matchBuyers(session.orgId, deal);
  const writable = can(session, "buyer:write");
  return (
    <>
      <PageHeader crumbs={[{ label: "Deal Analyzer", href: "/analyzer" }, { label: `v${detail.analysis.version}`, href: `/analyzer/${analysisId}` }, { label: "Match buyers" }]} title={`Buyers for ${detail.property.addressLine1}`} description={`Investor price ${money(deal.investorBuyPrice)} · all in ${percent(deal.arv ? (deal.investorBuyPrice + deal.repairCosts) / deal.arv : 0)} of ARV · spread ${money(deal.spread)} · ${detail.property.propertyType ?? "type unknown"} in ${detail.property.county ?? detail.property.city}`} />
            ...
            <CardBody className="flex flex-col md:flex-row md:items-center gap-3">
              ...
              {writable ? (
                <ActionForm action={recordSubmission.bind(null, analysisId, m.buyer.id)} submitLabel="Mark sent" size="sm" variant="outline" inline className="flex items-center gap-1 shrink-0">
                  <input type="hidden" name="packageId" value={sp.package ?? ""} />
                  <Select name="sentVia" defaultValue="email" className="w-24 h-7 text-xs"><option value="email">Email</option><option value="text">Text</option><option value="phone">Phone</option></Select>
                </ActionForm>
              ) : null}
            </CardBody>
          </Card>
