...
export async function enrichProperty(orgId: string, propertyId: string, actorId: string | null) {
  ...
  // The comps call is priced separately from the lookup; both charges land on the same report row so the budget ledger sees the full cost of the run.
  if (compsCostCents) await db.update(propertyReports).set({ costCents: sql`${propertyReports.costCents} + ${compsCostCents}` }).where(eq(propertyReports.id, report!.id));
