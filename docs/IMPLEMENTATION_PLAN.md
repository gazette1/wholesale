...
11. **Decisions that belong to Ous.** Approving the project model so it can replace the workbook's Flip P&L; the project model rules in `docs/MAC_PARITY.md` (cash flow and flip now read from the Mac binary, the rental projection still partly inferred); the 34 workbook anomalies in `spec/ANOMALIES.md`; and which outside tools to connect first, which decides whether any native adapter is worth building.
12. **The rest of the Mac app's rental math.** The cash flow and flip rules were read from the Mac binary on 2026-09-21 and the engine follows them. The rental projection is still partly inferred: tax savings, appreciation booking, debt service after payoff, the rent case, and rounding. Either finish reading `Calculations.rental` in the binary, get the Swift source (`~/Desktop/projects/DealAnalyzer/Sources/`), or run one rental deal through both apps and compare. Details in `docs/MAC_PARITY.md`.
